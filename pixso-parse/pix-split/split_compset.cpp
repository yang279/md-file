...
int main(int argc, char **argv) {
    ...
    if (cmd == "report") {
        ...
        // A2：输出子目录为 <outdir>/component/
        std::string compDir = outdir + "/component";
        mkdir(outdir.c_str(),  0755);
        mkdir(compDir.c_str(), 0755);

        for (size_t i = 0; i < sets.size(); i++) {
            const CompSet &cs = sets[i];
            if (cs.pixData.empty()) continue;

            // A2：文件名优先使用 componentKey，无则用 guid
            std::string fname = cs.componentKey;
            if (fname.empty())
                fname = std::to_string(cs.rootGuid.s) + "_" + std::to_string(cs.rootGuid.l);

            std::string hex  = "<!-- pixso binary data -->\n" + bytesToHex(cs.pixData);
            std::string path = compDir + "/" + fname + ".txt";
        ...
        // A3：build_index 额外生成 component_index.json
        if (cmd == "build_index") {
            std::string idxPath = compDir + "/component_index.json";
            FILE *jf = fopen(idxPath.c_str(), "w");
            if (!jf) { fprintf(stderr, "cannot write: %s\n", idxPath.c_str()); return 1; }
            ...
            for (auto &cs : sets) {
                ...
                firstCs = false;
                std::string hexFile = "component/" + (cs.componentKey.empty()
