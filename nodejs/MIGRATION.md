...
两者**不再是网络调用关系**：`dsl-to-hex` 转换时按设计 DSL 中 instance 的 `path` 字段直接拼本地路径读取 hex 文件，不再通过 HTTP 请求 `component-service` 的 `GET /hex/:key`。但二者都依赖同一份 `lib-out` 组件库数据（分别通过 `LIB_OUT_DIR` / `HEX_LIB_DIR` 配置指向），因此**推荐整体一起迁移**，并确保两个服务的目录配置指向同一份 `lib-out` 数据。

---

## 一、component-service（端口 3102）
...
PORT=3102 LIB_OUT_DIR=/srv/data/lib-out node server.js
```
...
curl http://localhost:3102/hex/93_55829           # h-design-chart 的 key，验证跨库查找
curl http://localhost:3102/hex/<ict-ui的某个SHA1key>  # ict-ui 的 key
...
|---|---|
| `server.js` | HTTP 服务入口 |
| `converter.js` | 核心转换逻辑（提取组件 key/path、读取本地 hex、调用 WASM、打包 zip） |
| `package.json` | 无外部 npm 依赖 |
| `.env` | 含 `HEX_LIB_DIR` 配置（非敏感信息，可一起带走或在目标机器上重新写） |
| `bin/dsl_to_hex.js` + `bin/dsl_to_hex.wasm` | **WASM 转换核心，必须两个文件成对存在、放在同一目录** |

> `API.md`、`test-data/` 为文档与测试数据，非运行必需，可选择性带走。

### 2.2 ⚠️ 必须额外迁移的外部数据：lib-out 组件库

`dsl-to-hex` 按设计 DSL 中 instance 的 `path` 字段（形如 `"h-design-chart/component/93_55829.txt"`）直接拼接 `HEX_LIB_DIR + path` 读取本地 hex 文件——**与 component-service 依赖的是同一份 `lib-out` 数据**（见上文 1.2），无需重复迁移，但要确保 `HEX_LIB_DIR` 指向的路径与 `LIB_OUT_DIR` 是同一目录（或同一份数据的另一份拷贝）。
...
|---|---|---|---|
| `PORT` | `3101` | 按需 | 监听端口 |
| `HEX_LIB_DIR` | `../../pixso-parse/pix-split/lib-out`（相对 `__dirname` 解析） | **是** | 迁移后原相对路径大概率失效，需指向新机器上 lib-out 的实际绝对路径，且应与 component-service 的 `LIB_OUT_DIR` 指向同一份数据 |
...
```bash
cd dsl-to-hex
PORT=3101 HEX_LIB_DIR=/srv/data/lib-out node server.js
```
...
# instance.path 缺失，或 HEX_LIB_DIR 指向的 lib-out 数据不全/路径配错）
```
...
□ dsl-to-hex/ 整个目录（含 .env、bin/ 下的 .js + .wasm 必须成对）
□ 目标机器已安装 Node.js（≥ 18 推荐）
□ 目标机器已安装系统 zip 命令（dsl-to-hex 打包依赖）
□ 目标机器可访问 https://api.deepseek.com（component-service LLM 调用）
□ 设置 LIB_OUT_DIR（component-service）和 HEX_LIB_DIR（dsl-to-hex）指向同一份 lib-out 数据
...
| `/convert` 返回结果带 `missing_keys` | DSL 中 instance 缺少 `path` 字段，或 `HEX_LIB_DIR + path` 拼出的本地文件不存在——检查 `HEX_LIB_DIR` 配置及对应 `lib-out/{source}/` 目录是否完整迁移 |
